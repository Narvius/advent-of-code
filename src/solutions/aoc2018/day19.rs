...
const OPS: [(&str, Op); 16] = [
    ...
    ("setr", |ra, _, _, _| ra),
    ("seti", |_, va, _, _| va),
    ("gtir", |_, va, rb, _| if va > rb { 1 } else { 0 }),
    ("gtri", |ra, _, _, vb| if ra > vb { 1 } else { 0 }),
    ("gtrr", |ra, _, rb, _| if ra > rb { 1 } else { 0 }),
    ("eqir", |_, va, rb, _| if va == rb { 1 } else { 0 }),
    ("eqri", |ra, _, _, vb| if ra == vb { 1 } else { 0 }),
    ("eqrr", |ra, _, rb, _| if ra == rb { 1 } else { 0 }),
];
